...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if(!message.member.voice.channel?.permissionsFor(message.guild.members.me)?.has(Permissions.FLAGS.CONNECT)) {
            return message.reply({ content: "**I'm missing the Permission to Connect to your Voice-Channel!**"}).catch(() => null);
        }
        if(!message.member.voice.channel?.permissionsFor(message.guild.members.me)?.has(Permissions.FLAGS.SPEAK)) {
