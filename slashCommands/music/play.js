...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!interaction.member.voice.channel?.permissionsFor(interaction.guild.members.me)?.has(Permissions.FLAGS.CONNECT)) {
            return interaction.reply({ephemeral: true, content: "**I'm missing the Permission to Connect to your Voice-Channel!**"}).catch(() => null);
        }
        if(!interaction.member.voice.channel?.permissionsFor(interaction.guild.members.me)?.has(Permissions.FLAGS.SPEAK)) {
